...
Public Class frmMain
    ...
    Private Sub btnOpen_Click(sender As Object, e As EventArgs) Handles btnOpen.Click
        If RadioButton1.Checked = True Then 'eğer tek bir dosya açacaksa dosya seç dialogu gelecek.
            dlgOpen.ShowDialog()
            If dlgOpen.FileName.Trim <> "" And dlgOpen.FileName.Trim <> txtFrom.Text Then txtFrom.Text = dlgOpen.FileName.Trim : txtTo.Text = txtFrom.Text.Replace(".avi", ".mp4").Replace(".flv", ".mp4").Replace(".wmv", ".mp4") 'eğer dosya adı ok ise
        Else 'yoksa klasör seçtirip oradaki tüm avileri dönüştürecek.
            ...
            If dlgFolder.SelectedPath <> "" And dlgFolder.SelectedPath <> txtFrom.Text Then txtFrom.Text = dlgFolder.SelectedPath & "\*.*" : txtTo.Text = txtFrom.Text.Replace(".*", ".mp4")
        End If
    End Sub
    ...
    Private Sub btnConvert_Click(sender As Object, e As EventArgs) Handles btnConvert.Click
        ...
        Else 'burada çoklu dönüştürme işlemi yapılacak
            Dim directory As New IO.DirectoryInfo(txtFrom.Text.Replace("\*.*", "")) : Dim Files As IO.FileInfo() = directory.GetFiles() : Dim FileNames As IO.FileInfo : Dim eski(1) As String : Dim i As Integer = 0
            For Each FileNames In Files 'tüm aviler bir diziye atılır
                If FileNames.Extension = ".avi" Or FileNames.Extension = ".flv" Or FileNames.Extension = ".wmv" Then eski(i) = FileNames.FullName : i += 1 : ReDim Preserve eski(i)
            Next
            For j = 0 To i - 1
                Dim ad() As String = eski(j).Split("\") : Dim ad2 As String = txtTo.Text.Replace("*.mp4", ad(UBound(ad))).Replace(".avi", ".mp4").Replace(".flv", ".mp4").Replace(".wmv", ".mp4")
